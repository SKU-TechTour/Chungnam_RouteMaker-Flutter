...
`military_guide`만 네이티브 연동을 위해 추가 레이어가 있습니다:
...
`main.dart`는 **테마, 라우팅, DI만 연결**하는 진입점입니다. 비즈니스 로직은 feature에만 존재합니다.
...
모든 의존 관계는 `core/di/providers.dart`에서 한 번에 정의됩니다.
...
|---|---|---|---|
| home_curation | `GET` | `/api/courses` | 3단 콤보 코스 목록 |
| military_guide | `GET` | `/api/military/safe-time` | 복귀 가능까지 남은 시간(분) |
| map_search | `POST` | `/api/places/filter` | GPS + 필터 조건으로 장소 검색 |
| my_history | `GET` | `/api/rewards` | 서버 리워드/영수증 목록 |
...
> `baseUrl`은 현재 `http://localhost:8080`입니다.  
> 실기기 테스트 시 `--dart-define` 또는 flavor로 dev/staging/prod 분리하세요.

### 5.2 Flutter ↔ Native (MethodChannel)

`military_guide` feature에서 Android/iOS **라이브 위젯**과 통신합니다.
...
`my_history` feature에서 **로컬 스탬프/뱃지**를 기기 내에 저장합니다.
...
ViewModel이 **원격 + 로컬 Repository를 조합**하여 하나의 State로 View에 전달합니다.
...
GPS 권한 요청·좌표 획득은 `core/utils/location_util.dart`에서 처리하고, ViewModel은 결과만 사용합니다.
...
| 파일 | 계층 | 역할 |
|---|---|---|
| `models/course.dart` | Model | 3단 콤보 코스 (`id`, `title`, `spots`, `weatherTag`) |
| `repositories/course_repository.dart` | Repository | `GET /api/courses` |
| `viewmodels/home_curation_state.dart` | State | courses, currentIndex, isLoading, error |
| `viewmodels/home_curation_viewmodel.dart` | ViewModel | loadCourses, shuffleToNext, onSwipe |
| `views/home_screen.dart` | View | 메인 화면 |
| `views/widgets/combo_card.dart` | View | 코스 카드 UI |
...
| 파일 | 계층 | 역할 |
|---|---|---|
| `data/platform/live_widget_channel.dart` | Platform | MethodChannel 네이티브 통신 |
| `repositories/military_repository.dart` | Repository | API + 라이브 위젯 동기화 |
| `viewmodels/military_guide_viewmodel.dart` | ViewModel | 1초 Timer 카운트다운 |
| `views/countdown_widget.dart` | View | 카운트다운 UI |

**특이사항:** `CountdownWidget`은 `main.dart`의 `builder`에도 삽입되어 **모든 화면 하단**에 표시됩니다 (외부 위젯 SB 요구사항).
...
| 파일 | 계층 | 역할 |
|---|---|---|
| `models/place.dart` | Model | Place + PlaceFilterRequest |
| `repositories/place_repository.dart` | Repository | `POST /api/places/filter` |
| `viewmodels/map_search_viewmodel.dart` | ViewModel | petFriendly/strollerAccessible 필터 |
| `views/map_screen.dart` | View | 지도 화면 (지도 SDK 연동 지점) |
| `views/filter_bottom_sheet.dart` | View | 필터 바텀시트 |
...
| 파일 | 계층 | 역할 |
|---|---|---|
| `models/receipt.dart` | Model | 영수증 카드 (서버) |
| `models/stamp.dart` | Model | 스탬프/뱃지 (로컬) |
| `repositories/reward_remote_repository.dart` | Repository | `GET /api/rewards` |
| `repositories/stamp_local_repository.dart` | Repository | SharedPreferences |
| `viewmodels/my_history_viewmodel.dart` | ViewModel | loadHistory, shareReceipt |
| `views/receipt_share_screen.dart` | View | 원터치 공유 |
| `views/stamp_history_screen.dart` | View | 스탬프 이력 |
...
`go_router` 기반. 모든 라우트는 `core/routing/app_router.dart`에 정의됩니다.

| 경로 | 화면 | Feature |
|---|---|---|
| `/` | HomeScreen | home_curation |
| `/map` | MapScreen | map_search |
| `/history/receipt` | ReceiptShareScreen | my_history |
| `/history/stamps` | StampHistoryScreen | my_history |
...
| 패키지 | 버전 | 용도 |
|---|---|---|
| `flutter_riverpod` | ^2.6.1 | ViewModel(Notifier) + DI(Provider) |
| `dio` | ^5.8.0 | Spring HTTP 통신 |
| `go_router` | ^15.1.2 | 선언적 라우팅 |
| `geolocator` | ^14.0.1 | GPS 좌표 획득 |
| `shared_preferences` | ^2.5.3 | 로컬 스탬프 저장 |
...
3. `core/di/providers.dart`에 Repository Provider + ViewModel NotifierProvider 등록
4. `core/routing/app_router.dart`에 GoRoute 추가
5. (선택) `test/features/{name}/`에 ViewModel·Repository 단위 테스트 작성
...
- 상세 아키텍처 가이드: [ARCHITECTURE.md](./ARCHITECTURE.md)
...
| 브랜치 | 설명 |
|---|---|
| **`main`** | GitHub 기본 브랜치 — Flutter 프로젝트 전체 코드가 여기에 있습니다 |
| **`master`** | `main`과 동일 내용 유지 (레거시) |

> 앞으로 작업·push는 **`main`** 브랜치 기준으로 진행하세요.
